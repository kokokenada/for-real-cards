...
export function gamePlayReducer(oldState: IGamePlayRecord = INITIAL_STATE,
  ...
  switch (action.type) {
    case GamePlayActions.GAME_PLAY_ACTION_RECIEVED: {
      ...
      newState = newState.withMutations( (transient:IGamePlayRecord)=> {
        console.log('PROCESSING GAME PLAY ACTION');
        console.log(GamePlayActionType[gamePlayAction.actionType]);
        ...
        switch (gamePlayAction.actionType) {
          ...
          case GamePlayActionType.NEW_HAND: {
            ...
            let newHands: List<Hand> = newState.hands.push(newHand);
          ...
          case GamePlayActionType.DECK_TO_HAND:
            if (checkCards(gamePlayAction)) {
              ...
              gamePlayAction.cards.forEach((card: Card)=> {
                ...
                transient.set('tableFaceDown', newState.tableFaceDown.splice(index, 1));
              });
          ...
          case GamePlayActionType.HAND_TO_PILE: {
            ...
            if (checkCards(gamePlayAction)) {
              gamePlayAction.cards.forEach((card: Card)=> {
                let index: number = getIndexOfCard(hand.cardsInHand, card);
                transient.set('tablePile', newState.tablePile.push(card));
          ...
          case GamePlayActionType.DECK_TO_PILE: {
            let index = 0;
            transient.set('tablePile', newState.tablePile.push(newState.tableFaceDown.get(index)));
            transient.set('tableFaceDown', newState.tableFaceDown.splice(index, 1));
            break;
          }
          ...
          case GamePlayActionType.PILE_TO_HAND: {
            ...
            if (checkCards(gamePlayAction)) {
              gamePlayAction.cards.forEach((card: Card)=> { // Can this loop be collapsed into a single call?
                ...
                transient.set('tablePile', newState.tablePile.splice(index, 1));
              })
            }
            break;
          }
          case GamePlayActionType.PILE_TO_DECK: {
            if (checkCards(gamePlayAction)) {
              gamePlayAction.cards.forEach((card: Card)=> {  // Can this loop be collapsed into a single call?
                let index: number = getIndexOfCard(newState.tablePile, card);
                transient.set('tableFaceDown', newState.tableFaceDown.push(card));
                transient.set('tablePile', newState.tablePile.splice(index, 1));
              });
            }
            break;
